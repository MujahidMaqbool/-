...
@Component({
    ...
    <div class="appointment-content" [ngStyle]="{'background': lightenTheColor}">
        ...
        <div class="float-right" *ngIf="isSaveTaskAllowed">
            <dx-button *ngIf="!data.MarkAsDone" class="delete-appointment" (onClick)="onEditTask()"><i class="fal fa-edit"></i></dx-button>
            <dx-button *ngIf="!data.MarkAsDone" class="delete-appointment" (onClick)="onDeleteTask()" ><i class="fal fa-trash-alt"></i></dx-button>
    ...
    <div class="row appointment-content-detail">
        <div class="col-md-6 col-sm-6 no_padding text-left">
            <div class="appointment_item_detail">
                <h3>Start Date</h3>
                <p><i class="fal fa-calendar-alt"></i> {{(data.startDate | customdate : schedulerTooltipDateFormat) + ' - ' + (data.startDate | customdate: schedulerTimeFormat)}}</p>
            </div>
            <div class="appointment_item_detail">
                <h3>Priority</h3>
                <p>{{data.PriorityTypeName}}</p>
            </div>
            <div class="appointment_item_detail">
        ...
        <div class="col-md-6 col-sm-6 padding_left text-left xs-pl-0">
            <div class="appointment_item_detail">
                <h3>End Date</h3>
                <p><i class="fal fa-calendar-alt"></i> {{(data.endDate | customdate : schedulerTooltipDateFormat) + ' - ' + (data.endDate | customdate: schedulerTimeFormat)}}</p>
            </div>
            <div class="appointment_item_detail">
                <h3>Assigned To</h3>
...
export class TaskTooltipComponent extends AbstractGenericComponent implements OnInit {
    @Input() data: any;
